...
var googleLogo=[
	...
	' %c(blue)HH    HH %c(red)AA          AA %c(yellow)PP        %c(yellow)PP          %c(green1)YY  %c(0)',
	'',
	' %c(yellow)BBBBBBB     %c(cyan)c;    %c(green1)DDDDDDD          %c(blue) A        %c(red)YY    YY   %c(0)',
...
function compileHomeScreen() {
	...
	homeScreen.push(frameLine);
	homeScreen.push( pad(10, '%c(green2)Welcome, it\'s ' + logDate + '. It\'s your Birthday! :D%c(0)') ); //'+logDate+'
	homeScreen.push(frameLine);
...
function termInitHandler() {
	this.type('Nishant@Nishant-PC:~/xperis/Tyler.Durden$');
	this.cursorOn();
	var opts='';
